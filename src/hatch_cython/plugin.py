...
class CythonBuildHook(BuildHookInterface):
    ...
    @property
    def autogenerated_files(self) -> ListStr:
        return list(filter(os.path.exists, (s.replace(".in", "") for s in list(self.templated_files.keys()))))
    ...
    def clean(self, versions: ListStr) -> None:
        files_to_remove = self.autogenerated_files + list(self.intermediate_files.values()) + list(self.compiled_files.values())
        ...
        self.app.display_info(f"Hatch-cython: intermediate {list(self.intermediate_files.values())}")

...
def remove_leading_dotslash(path: str) -> str:
    return path.replace("./", "")
    ...
    def initialize(self, version: str, build_data: dict):
        ...
        if len(self.excluded) > 0:
            if "exclude" not in build_data:
                ...
                # removing leading ./ from paths as local fix because hatch requires git style glob patterns
            self.build_config.target_config["exclude"].extend(
                [remove_leading_dotslash(f) for f in self.excluded]
            )
